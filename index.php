...
$check1 = json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@a3&user_id=$from_id"))->result->status;
$check2 = json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@a3&user_id=$from_id"))->result->status;
$timee = json_decode(file_get_contents("http://api-bot-telegram.cf/api/time.php?token=a1"));
$time2 = objectToArrays($timee);
$time = $time2["FAtime"];
$date = $time2["FAdate"];
$chistan = file_get_contents("http://api-bot-telegram.cf/api/chistan.php?token=a1");
...
$check11 = json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@a3&user_id=$chatid"))->result->status;
$check22 = json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@a3&user_id=$chatid"))->result->status;
...
یا ایدی درستی وارد نمایید یا به صفحه قبلی برگردید و کانال خود را ثبت کنید🔰",
            ...
            sendAction($chat_id, 'typing');
$admin = getChatstats(@$text,"توکن ربات");
if($admin != true){
          bot('sendmessage', [
            'chat_id' => $chat_id,
...
توجه : باید این ربات حتما در کانال تان ادمین باشد🤗",
 ...
 bot('sendMessage',[
    ...
    sendAction($chat_id, 'typing');
  $url = 'https://api.telegram.org/botتوکن ربات/getChatAdministrators?chat_id=@'.$text;
$admin = getChatstats(@$text,"توکن ربات");
if($admin != true){
          bot('sendmessage', [
            'chat_id' => $chat_id,
